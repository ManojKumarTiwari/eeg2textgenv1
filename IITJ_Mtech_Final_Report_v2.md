...
**Figure 3.1**: The encoder layer computes attention scores between all input tokens, then passes through a 2-layer FFN with a non-linear activation (GELU or SwiGLU).
...
**Figure 3.2**: LoRA inserts parallel trainable matrices BA alongside frozen W₀. The downstream output is the sum of the frozen path and the low-rank update path.

```mermaid
flowchart LR
    X["Input x"]

    subgraph FROZEN["Frozen Path (no gradient)"]
        W0["W₀\nd × k\nfrozen weights"]
    end
    ...
    style LORA fill:#e8f4ff,stroke:#4a90d9
```
...
```mermaid
flowchart TD
    EEG_IN["Raw EEG Signal\n(B, 22 channels, 4 patches, 200 samples/patch)"]

    subgraph CSB["CSBrain Encoder — FROZEN  (~12M params)"]
        direction TB
        CB1["PatchEmbedding\nConv2D + Positional + Spectral (FFT) Embeddings"]
        CB2["TemEmbedEEGLayer\nCross-scale temporal convolutions: kernels 1, 3, 5"]
        CB3["BrainEmbedEEGLayer\nRegion-aware spatial embedding"]
        CB4["12 × CSBrain Transformer Layer\nInter-window attention ↔ Inter-region attention"]
        CB5["Output: (B, 22, 4, 200)\nd_model = 200 per channel-patch"]
        CB1 --> CB2 --> CB3 --> CB4 --> CB5
    end

    subgraph RED["EEGTokenReducer — No Parameters"]
        direction TB
        R1["Group 22 channels → 3 brain regions\nFrontal (1ch) · Central (17ch) · Parietal (4ch)"]
        R2["Average-pool channels within each region\n(B, 22, 4, 200) → (B, 3, 4, 200)"]
        R3["Flatten region × patch dimensions\n(B, 3, 4, 200) → (B, 12, 200)   [12 EEG tokens]"]
        R1 --> R2 --> R3
    end

    subgraph PROJ["EEGProjection MLP — TRAINABLE  (~4.6M params)"]
        direction TB
        P1["Linear ( 200 → 2048 )"]
        P2["GELU activation"]
        P3["Dropout ( p = 0.1 )"]
        P4["Linear ( 2048 → 2048 )"]
        P5["Output: (B, 12, 2048)   [LLM hidden dim]"]
        P1 --> P2 --> P3 --> P4 --> P5
    end

    PROMPT["Prompt Text Embeddings\n(B, 101, 2048)\nSystem + User instruction"]
    TARGET["Target Description Embeddings\n(B, 61, 2048)\nNeuroscience label text"]

    subgraph LLM["TinyLlama-1.1B-Chat — 4-bit NF4 Quantized"]
        direction TB
        L1["Concatenate along sequence dim\n[ Prompt (101) | EEG (12) | Target (61) ] = 174 tokens"]
        L2["22 × Transformer Decoder Layers\nRoPE · RMSNorm · SwiGLU · GQA"]
        L3["LoRA Adapters on q_proj & v_proj — TRAINABLE (~1.1M params)"]
        L4["Language Model Head → next-token logits"]
        L5["Cross-entropy loss on Target tokens only\n(Prompt + EEG positions masked with label −100)"]
        L1 --> L2 --> L3 --> L4 --> L5
    end

    OUT["Generated Text\ne.g. 'The EEG shows left hand motor imagery\nwith right-hemisphere mu/beta desynchronization'"]

    EEG_IN --> CSB --> RED --> PROJ --> LLM
    PROMPT --> LLM
    TARGET --> LLM
    LLM --> OUT

    style CSB fill:#fff3cd,stroke:#e6a817
    style RED fill:#d4edda,stroke:#28a745
    style PROJ fill:#cce5ff,stroke:#004085
...
```mermaid
flowchart TD
    IN["EEG Input\n(B, 22, 4, 200)\n22 channels · 4 temporal patches · 200 samples"]

    SORT["Channel Re-sort by Brain Region\nFrontal → Central → Parietal\nindices: [0, 18,19,20,21, 1..17]"]

    subgraph PE["PatchEmbedding"]
        PE1["Conv2D Patch Projection\nraw samples → d_model features"]
        PE2["Positional Embeddings\npatch position encoding"]
        PE3["Spectral Embeddings\nFFT-derived frequency encoding"]
        PE4["Element-wise Summation"]
        PE1 --> PE4
        PE2 --> PE4
        PE3 --> PE4
    end

    subgraph TEM["TemEmbedEEGLayer  (Cross-Scale Temporal)"]
        direction LR
        K1["Conv kernel=1\nlocal / instantaneous"]
        K3["Conv kernel=3\nshort-range temporal"]
        K5["Conv kernel=5\nlong-range temporal"]
        AGG["Learnable Weighted Aggregation"]
        K1 & K3 & K5 --> AGG
    end

    subgraph BEM["BrainEmbedEEGLayer  (Spatial)"]
        BEM1["Region-aware inter-channel attention\nFrontal · Central · Parietal grouping"]
    end

    subgraph TRANS["12 × CSBrain Transformer Encoder Layer"]
        direction TB
        IW["Inter-window Attention\ntemporal dependencies within each brain region"]
        IR["Inter-region Attention\nspatial dependencies across brain regions\n(structured sparsity mask)"]
        FFN["Feed-Forward Network\nGELU + residual + layer norm"]
        IW --> IR --> FFN
    end

    OUT["Encoder Output\n(B, 22, 4, 200)\nd_model=200 per channel-patch token"]

    IN --> SORT --> PE --> TEM --> BEM --> TRANS --> OUT

    style PE fill:#fff9db,stroke:#e6c200
    style TEM fill:#e8f5e9,stroke:#388e3c
    style BEM fill:#e3f2fd,stroke:#1565c0
...
flowchart LR
    ...
    subgraph REGIONS["Anatomical Region Grouping"]
        direction TB
        F["Frontal\n1 channel\nFz"]
    ...
    subgraph POOL["Average Pooling per Region"]
        direction TB
        FP["Mean over 1 ch → (B,1,4,200)"]
        CP["Mean over 17 ch → (B,1,4,200)"]
        PP["Mean over 4 ch → (B,1,4,200)"]
    end

    STACK["Stack regions\n(B, 3, 4, 200)"]
    FLAT["Flatten region × patch\n(B, 12, 200)\n12 EEG tokens"]
    ...
    style REGIONS fill:#e8f5e9,stroke:#2e7d32
...
```mermaid
flowchart LR
    subgraph SEQ["Input Sequence  (174 tokens total)"]
        direction LR
        PT["PROMPT TOKENS\n101 tokens\nSystem + User text\nvia tokenizer embedding"]
        ET["EEG TOKENS\n12 tokens\nfrom EEGProjection\nshape: (12, 2048)"]
        TT["TARGET TOKENS\n61 tokens\nNeuroscience description\nvia tokenizer embedding"]
        PT --> ET --> TT
    end

    subgraph LABELS["Loss Labels"]
        direction LR
        PM["label = −100\nmasked from loss"]
        EM["label = −100\nmasked from loss"]
        TM["label = token_ids\nsupervised loss"]
    ...
    style PT fill:#fff3cd,stroke:#e6a817
    style ET fill:#cce5ff,stroke:#004085
    style TT fill:#d4edda,stroke:#155724
    style PM fill:#f5f5f5,stroke:#999
    style EM fill:#f5f5f5,stroke:#999
    style TM fill:#d4edda,stroke:#155724
```
...
```mermaid
gantt
    title EEG2Text Two-Phase Training Strategy (20 Epochs)
    dateFormat  X
    axisFormat  Epoch %s

    section Phase 1 — Projection Warmup
    EEGProjection  (LR = 1e-3, constant)  : active, p1, 1, 6

    section Phase 2 — Joint Fine-Tuning
    EEGProjection  (LR = 2e-4 → 1e-6, cosine) : active, p2a, 6, 21
    LoRA Adapters  (LR = 2e-4 → 1e-6, cosine) : active, p2b, 6, 21

    section Validation Accuracy
    Best checkpoint — Epoch 6 (36.81%)         : milestone, m1, 6, 7
```
...
```mermaid
timeline
    title Single Trial Timeline (BCIC-IV-2a)
    section 0 s
        Fixation cross appears
    section 2 s
        Auditory beep cue
    section 3 s
        Visual arrow cue (MI class)\nMotor Imagery window begins
    section 7 s
        End of MI window (optional)
    section 10.5 s
        Rest / break period
```
...
```mermaid
flowchart TD
    RAW["Raw .mat File\nA01T.mat ... A09E.mat\nMATLAB format from BNCI Horizon 2020"]

    S1["1. Load EEG + Labels\nscipy.io.loadmat\nExtract 22 EEG channels, ignore 3 EOG channels"]

    S2["2. Zero-mean Normalization\nx = x − x.mean()\nRemoves slow DC offset per trial"]

    S3["3. Bandpass Filter: 0.3 – 50 Hz\n5th-order Butterworth\nzero-phase forward-backward (filtfilt)\nRemoves drift + power-line noise (50 Hz)"]

    S4["4. Epoch Extraction: [2 s, 6 s] post-cue\n750 – 1750 samples at 250 Hz\n→ 1000 samples per trial"]

    S5["5. Resample: 250 Hz → 200 Hz\n1000 samples → 800 samples\nPyTorch FFT-based resampling"]

    S6["6. Reshape: (22, 800) → (22, 4, 200)\nDivide 800 samples into\n4 temporal patches × 200 samples"]

    S7["7. Amplitude Normalization\ndivide by 100\nBrings ~100 µV EEG to ~1.0 (unit scale)"]

    S8["8. Store to LMDB\nkey: '{split}_{index}'  e.g. 'train_1023'\nvalue: pickle({'sample': ndarray, 'label': int})\nO(1) random access for DataLoader"]

    RAW --> S1 --> S2 --> S3 --> S4 --> S5 --> S6 --> S7 --> S8

    style RAW fill:#f5f5f5,stroke:#666
    style S3 fill:#fff3cd,stroke:#e6a817
...
Pre-aligning the CSBrain encoder outputs with the LLM embedding space through a contrastive objective (EEG-text pairs) before the main training could provide a better starting point for the projection warmup phase.
